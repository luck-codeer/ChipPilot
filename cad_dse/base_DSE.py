...
if __name__ == "__main__":
    ...
    print("第一次单进程串行实际运行时间为：%.2f秒" % time_cost)
    result_str = "第一次单进程串行运行时间为：%.2f秒" % time_cost
    with open('BaseDSEReport.txt', 'w') as file:
        file.write(result_str + '\n')

    time_cost2 = time_cost*100 #time_cost即为程序运行的时间，单位是秒
    print("第一次单进程串行预测运行时间为：%.2f秒" % time_cost2)
    result_str = "第一次单进程串行预测运行时间为：%.2f秒" % time_cost2
